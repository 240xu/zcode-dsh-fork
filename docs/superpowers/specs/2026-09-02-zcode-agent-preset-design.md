...
- VALIDATION 回顾（分类：自创校验）：首版对空命令/非正 timeout/sandbox 标记抛错。oracle 活体：空命令短路 completed-empty（不 spawn，generic `(Bash completed with no output)` 括号规则）；timeout 0/-5 照跑（bundle `resolveBashTimeoutMs`：falsy→默认 120000，超大静默 cap 到 600000，永不报错，另有 `BASH_*_TIMEOUT_MS` env 可配）；sandbox 标记 yolo 下接受无效果。已改写（短路/默认-cap/接受忽略），server2 12/12。差异：负 timeout 实际效果、cap 下 timeout 文案用请求值还是有效值、超时 partial-stdout 是否计入，待探；`BASH_*` env 本实现用固定常量。
- TIMEOUT-MACHINE 回顾（分类：缺失行为）：oracle 前台超时对 eligible 命令（非空且首词非 sleep）不是 kill，而是转后台继续跑（`auto_on_timeout` 前台 deadline → commitBackground；`sleep` 首词/无 lifecycle 才走 kill）。另确认：非正 timeout 在 auto 模式走无 timer 纯前台；`captureCwdAfterSuccess` 仅前台，后台永不 adopt（此前实现沿用累积解析 adopt，已删除）；sandbox flag 会流入执行层配置（受限部署下与 DSH host-controlled 冲突，记平台差异）。已实现三路由（explicit/auto/plain）+ stderr-file 分流（免混合 delta 二次切分）+ 改写版后台 ack（id 与收集通道取自本机 seam），server2 14/14。差异：后台 ack 文案与 `bash-N`/`exec_uuid`（平台 seam，适配记录）；cap 下文案值（静态已确认为 cap 后值，10 分钟活体确认待补）；cancelled/spawn 内容；前台截断契约；signal-kill 形状。
