...
可替换性有一条值得精确表述的边界：整行替换（适配器、工具集、提示词节、scoped 遮蔽，乃至 agent factory）是与现有驱动协作组合，但驱动自身的机制——私有的 turn/step 状态机、内部调度器符号、会话日志投影及其“模型可见即已记录”不变式、路由绑定与持久化、封闭的审批结果映射、属主相关的 id 注册表——归内核所有，只能在其文档化的 waterfall 处拦截，`defineTool` 插件永远无法重新拥有。从工具内部复现另一个循环的内部机制，会把这条属主箭头倒过来；在工具层面达成的行为等价，不蕴含内部架构等价。
