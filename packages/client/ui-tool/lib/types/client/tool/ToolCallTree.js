...
const ToolCall = memo(function ToolCall({ renderSlot, callId, toolName, block, openFile, selected, cwd, home, inspectCall, loadImage, t, children, }) {
    const owner = useMemo(() => ({
        callId,
        toolName,
        block,
...
const ToolCallBranch = memo(function ToolCallBranch({ renderSlot, block, selectedCallId, cwd, home, openFile, inspectCall, loadImage, t, }) {
    return (_jsx(ToolCall, { renderSlot: renderSlot, callId: block.callId, toolName: callName(block), block: block, openFile: openFile, selected: block.callId === selectedCallId, cwd: cwd, home: home, inspectCall: inspectCall, loadImage: loadImage, t: t, children: block.subCalls.length > 0 ? (_jsx("div", { className: css.subCalls, "data-subcalls": true, children: block.subCalls.map(child => (_jsx(ToolCallBranch, { renderSlot: renderSlot, block: child, selectedCallId: selectedCallId, cwd: cwd, home: home, openFile: openFile, inspectCall: inspectCall, loadImage: loadImage, t: t }, child.callId))) })) : null }));
});
/**
 ...
 */
export function ToolCallTree({ renderSlot, node, selectedCallId, cwd, openFile, inspectCall, loadImage, useHostInfo, t, }) {
    const home = useHostInfo(info => info.home);
    const block = node.data.root;
    return (_jsx(ToolCallBranch, { renderSlot: renderSlot, block: block, selectedCallId: selectedCallId, cwd: cwd, home: home, openFile: openFile, inspectCall: inspectCall, loadImage: loadImage, t: t }));
}
