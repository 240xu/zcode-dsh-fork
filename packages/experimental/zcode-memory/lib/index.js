...
function zcodeMemoryRoot() {
	return join(process.env.ZCODE_MEMORY_HOME ?? join(homedir(), ".zcode-agent"), "memory");
}
/** The rendered memory prompt: evidence text with the root substituted. */
function renderMemoryPrompt(root) {
	return rawPrompt.replaceAll("<MEMORY_ROOT>", root);
}
/** The full memory prompt with this deployment's root already substituted. */
const MEMORY_PROMPT = renderMemoryPrompt(zcodeMemoryRoot());
function apply(ctx) {
...
//#endregion
export { MEMORY_PROMPT, apply, inject, renderMemoryPrompt, zcodeMemoryRoot };
