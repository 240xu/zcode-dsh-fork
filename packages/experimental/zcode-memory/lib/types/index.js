...
export function zcodeMemoryScope() {
 ...
 */
export function zcodeStorageRoot() {
 ...
 */
export function zcodeMemoryRoot(scope = zcodeMemoryScope(), opts) {
    const agent = sanitizeAgentName(opts?.agentName ?? 'zcode');
    if (scope === 'user')
        return join(opts?.storageRoot ?? zcodeStorageRoot(), 'agent-memory', agent);
 ...
 */
export function formatMemoryIndex(raw) {
...
export const EMPTY_INDEX_MESSAGE = 'Your MEMORY.md is currently empty. When you save new memories, they will appear here.';
 ...
 */
export function renderMemoryPrompt(root, scope = zcodeMemoryScope(), indexContent) {
