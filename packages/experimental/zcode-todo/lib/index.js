...
const TODO_READ_DESCRIPTION = "Read the current session todo list";
const STATUSES = [
	"pending",
	"in_progress",
	"completed"
...
function apply(ctx) {
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_args, value) => [{
				type: "text",
				text: value.todos.length === 0 ? "(no todos)" : `Todo list: ${value.counts.pending} pending, ${value.counts.inProgress} in progress, ${value.counts.completed} completed.`
			}]
		},
		execute(_args, exec) {
...
//#endregion
export { TODO_READ_DESCRIPTION, apply, inject };
