/**
 * Child personas for the zcode `agent` multiplexer, verbatim from live
 * oracle runs (agent-server, embedded-search branch).
 *
 * - EXPLORE_PERSONA: specs/explore-persona-live.txt minus the host-injected
 *   `<env>` block and powered-by line (the host appends its own).
 * - GENERAL_PERSONA: runs/general-purpose-system-live.txt minus the same
 *   host-injected suffix.
 * @module @deepseek-ai/dsh-zcode-agent/personas
 */

export const EXPLORE_PERSONA = `You are ZCode, an interactive coding agent
...
export const GENERAL_PERSONA = `You are ZCode, an interactive coding agent
