...
var ApiSessionList = class {
	...
	*/
	...
	async probeSmallCold(header, signal) {
		...
		if (coldBlankProbeMaxEvents === 0 && coldBlankProbeMaxBytes === 0) return void 0;
		const persistence = this.ctx.get("sessionPersistence");
		if (persistence === void 0) return void 0;
		signal?.throwIfAborted();
		let snapshot;
		try {
			snapshot = await persistence.stat(header.id, signal === void 0 ? {} : { signal });
		} catch (error) {
			signal?.throwIfAborted();
			this.ctx.logger.warn(`api-session.list: cold stat for "${header.id}" failed; serving it as visible: ${String(error)}`);
			return;
		}
		if (snapshot === void 0) return void 0;
		if (snapshot.eventCount !== void 0) {
			if (coldBlankProbeMaxEvents === 0 || snapshot.eventCount > coldBlankProbeMaxEvents) return void 0;
		} else if (snapshot.sizeBytes !== void 0) {
			if (coldBlankProbeMaxBytes === 0 || snapshot.sizeBytes > coldBlankProbeMaxBytes) return void 0;
		} else return;
		try {
			const env_1 = {
				stack: [],
...
let SessionController = (() => {
	...
	return class SessionController extends _classSuper {
		...
		static Config = z.object({
			coldBlankProbeMaxEvents: z.natural().default(16),
		...
		*/
		constructor(ctx, config, internals = {}) {
			...
			this.listState = new ApiSessionList(ctx, {
				coldBlankProbeMaxEvents: config.coldBlankProbeMaxEvents ?? 16,
				coldBlankProbeMaxBytes: config.coldBlankProbeMaxBytes ?? 1024
			});
