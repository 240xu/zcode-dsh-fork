...
let MessageFeedbackService = (() => {
	...
	return class MessageFeedbackService extends _classSuper {
		...
		*/
		async inspectSession(sessionId) {
			if (this.ctx.sessions.get(sessionId) === void 0) {
				if (await this.ctx.sessionPersistence.stat(sessionId) === void 0 && this.ctx.sessions.get(sessionId) === void 0) return rejected({
		...
		hasFeedbackTarget(observation, messageId) {
		...
		*/
		async ensureTargetDurable(observation) {
			const live = this.ctx.sessions.get(observation.meta.id);
			if (live !== void 0 && sameHeaderIdentity(live.header, observation.meta)) {
				if (!await this.ctx.sessions.flush(live)) throw new Error(`message-feedback: no durability listener participated for live session '${observation.meta.id}'`);
			}
